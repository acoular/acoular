...
bd = BeamformerDamas(freq_data=f, steer=st, r_diag=True, n_iter=100)
bdp = BeamformerDamasPlus(freq_data=f, steer=st, r_diag=True, n_iter=100)
...
bl = BeamformerClean(freq_data=f, steer=st, r_diag=True, n_iter=100)
...
for b in (bb, bc, be, bm, bl, bo, bs, bd, bcmf, bf, bdp, bgib):
    ...
    mx = L_p(map.max())
    imshow(L_p(map.T), origin='lower', vmin=mx - 15, interpolation='nearest', extent=g.extend())
    colorbar()
...
bct = BeamformerCleant(source=fct, steer=st, n_iter=20, damp=0.7)
...
bcts = BeamformerCleantSq(source=fcts, steer=st, n_iter=20, damp=0.7, r_diag=True)
...
for b in (cacht, cachts, cachct, cachcts):
    ...
    imshow(L_p(map.T), vmax=mx, vmin=mx - 15, origin='lower', interpolation='nearest', extent=g.extend())
    colorbar()
    title(('BeamformerTime', 'BeamformerTimeSq', 'BeamformerCleant', 'BeamformerCleantSq')[i2 - 5])
tight_layout()
