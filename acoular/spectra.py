...
class PowerSpectra(BaseSpectra):
    ...
    #: :attr:`~acoular.spectra.BaseSpectra.block_size` and
    #: :attr:`~acoular.spectra.BaseSpectra.overlap` parameters. (read-only)
    num_blocks = Property()
    ...
    #: for ``n`` frequencies and ``m`` channels as in
    ...
    def calc_csm(self):
        ...
        :obj:`numpy.ndarray`
            ...
            for ``n`` frequencies and ``m`` channels as in
            :attr:`~acoular.spectra.BaseSpectra.num_channels`.
...
class PowerSpectraImport(PowerSpectra):
    ...
    #: :class:`~acoular.spectra.PowerSpectraImport` does not consume time data;
