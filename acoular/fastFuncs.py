...
@nb.guvectorize([(nb.float64[:], nb.float64[:], nb.float64[:], nb.float64[:], nb.float64[:], nb.float64[:], nb.float64[:],  
                  nb.float64[:], nb.float64[:], nb.float64[:], nb.float64[:,:], nb.float64[:], nb.complex128[:]),  
...
def greens_func_Induct(m, alphaAbs, axWaveNumWithFlow, axWaveNumAgainstFlow, axWaveNumImag, normFac, micPhi, micZ, gridPhi, gridZ, besselMic, besselGrid, result): 
        ...
        WATCH OUT: In the current numba version=0.35, bessel functions were not 
        implemeted yet. Therefor, they are calculated outside (via scipy.special)  
        and passed to this method. This means, lots of data must be written to  
        the cache. If bessel funcs are part of numbas supported numpy features  
        in the future, it may be worthy to evaluate the bessel funcs in here. 
         
        Even though the inputs can be double/float-precision, the float-precision 
        version doesn't seem to be faster at all. 
         
        Parameters 
        ---------- 
        m : float[nModes] 
            The Azimuthal mode orders. 
        alphaAbs : float[nModes] 
            Absolut value of alpha (see Phd-thesis of Ulf Tapken (TU Berlin).)
        axWaveNumWithFlow : float[nModes] 
            The axial wave number in flow direction, corresponding to m (real part)
        axWaveNumAgainstFlow : float[nModes] 
            The axial wave number against flow direction, corresponding to m (real part)
        axWaveNumImag : float[nModes] 
            The imaginary part of the axial wavenumbers, corresponding to m. Is the same for with/against flow. 
            Must be negativ in order to consider mode as cut-off.
        normFac: float[nModes] 
            Normalization factor of each mode 
        micPhi : float[nMics] 
            Phi-coordinates (cylindrical) of the mics. Must show into fan rotating direction. 
        micZ : float[nMics] 
            Z-coordinates (cylindrical) of the mics. Must show into flow direction. 
        gridPhi : float[nGridPoints] 
            Phi-coordinates (cylindrical) of the gridpoints. Must show into fan rotating direction. 
        gridZ : float[nGridPoints] 
            Z-coordinates (cylindrical) of the gridpoints. Must show into flow direction. 
        besselMic : float[nModes, nMics] 
            Bessel function for the various modes evaluated at relativ positions of the mics (J_n(sigma_mn * r / R)) 
        besselGrid : float[nGridPoints, nModes] 
            Bessel function for the various modes evaluated at relativ position of the grid points (J_n(sigma_mn * r / R) 
        result : complex128[nGridPoints, nMics] 
            The vector in which the calculated green functions are stored. 
        ...
        for cntMode in range(nModes): 
            besselSourceNormalized = besselGrid[cntMode] / (alphaAbs[cntMode] * normFac[cntMode])
            for cntMic in range(nMics): 
                dPhi = micPhi[cntMic] - gridPhi[0] 
                dZ = micZ[cntMic] - gridZ[0] 
                if dZ < 0.0:  # mic is UPstream of grid  
                    k = axWaveNumAgainstFlow[cntMode] 
                ...
                # if mode is cut-off --> multiply with analytical attenuation factor 
                if axWaveNumImag[cntMode] < -1e-12: 
                    modalTrans *= np.exp(np.float32(-abs(axWaveNumImag[cntMode] * dZ)))
                result[cntMic] += modalTrans
