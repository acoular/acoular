...
class BeamformerCleant( BeamformerTime ):
    ...
    def _fill_buffer(self,num):
        ...
        for block in self.source.result(num):
            ...
            self.buffer[0:(bufferSize-ns)] = self.buffer[-(bufferSize-ns):]  # copy remaining samples in front of next block
    ...
    def delay_and_sum(self,num,p_res,d_interp1,d_interp2,d_index,m_index): 
        ...
        for resIndex in range(num):
            result[resIndex]=(p_res[resIndex+d_index, m_index]*d_interp1 \
                        + p_res[resIndex+d_index+1, m_index]*d_interp2).sum(-1)
        return result
