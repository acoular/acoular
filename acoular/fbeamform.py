...
class BeamformerCMF(BeamformerBase):
    ...
    def _validate(self):
        if self.method in ['FISTA', 'Split_Bregman'] and not config.pylops_exists:
    ...
    def calc(self, ac, fr):
        ...
        for i in self.freq_data.indices:
            if not fr[i]:
                ...
                if self.method == 'Split_Bregman' and config.pylops_exists:
                ...
                elif self.method == 'FISTA' and config.pylops_exists:
