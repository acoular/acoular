...
UNEQUAL_DIGEST_TEST_DICT = {
    ...
    'IntegratorSectorTime.sectors item assignment': (
        IntegratorSectorTime(sectors=[Sector()]),
        'obj.sectors[0] = Sector()',
    ),
    'IntegratorSectorTime.sectors list assignment': (
        IntegratorSectorTime(sectors=[Sector()]),
        'obj.sectors = [Sector()]',
    ),
    # tprocess.py
    ...
    'Mixer.sources item assignment': (
        Mixer(source=SamplesGenerator(), sources=[SamplesGenerator()]),
        'obj.sources[0] = SamplesGenerator(numchannels=1)',
    ),
    'Mixer.sources list assignment': (
        Mixer(sources=[SamplesGenerator()]),
        'obj.sources = [SamplesGenerator(), SamplesGenerator()]',
    ),
