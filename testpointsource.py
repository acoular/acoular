...
r = 3
m.mpos_tot = array([(r*sin(2*pi*i),r*cos(2*pi*i),0) for i in linspace(0.0,1.0,28,False)]).T
xyz = m.mpos
print xyz
c0 = 343.0
inv_ch = []
...
p1 = PointSource(sample_freq=6144.0,numsamples=8192,mpos=m, loc=loc)
p2 = PointSource(sample_freq=6144.0,numsamples=8192,mpos=m, loc=loc1)
t = Mixer(source = p1, sources = [p2, ])
...
g = RectGrid(x_min=-3.0,x_max=+3.0,y_min=-3.0,y_max=+3.0,z=loc[2],increment=0.15)
