...
from enthought.chaco.api import ArrayDataSource, ArrayPlotData,\
 ...
 LinearMapper, Plot, VPlotContainer, jet , OverlayPlotContainer, LinePlot
from enthought.chaco.tools.api import ImageInspectorOverlay, ImageInspectorTool,\
 PanTool, RangeSelection, RangeSelectionOverlay, SaveTool #RectZoomTool, 
from enthought.chaco.tools.cursor_tool import BaseCursorTool, CursorTool1D 
...
# Bugfix for original CursorTool
class CursorTool(CursorTool1D):
    ...
    def draw(self, gc, view_bounds=None):
        ...
        # fixed to work with log axis
        sx, sy = plot.map_screen(numpy.array(self.current_position).reshape((1,2))).reshape(2)
...
#                            Action(name='&Frequency', action='set_Freq'),
    ...
    def __init__(self, **kwtraits):
        ...
        self.imgp = imgp
        self.map_plot.plot(("xpoly","ypoly"), name="sector", type = "line")
...
##         pd.set_data("freqs",ArrayDataSource(index,sort_order="ascending"))
##         pd.set_data("spectrum",value)
...
##         self.cursor.current_position = 0.1,0

##         print self.spec_plot,px
...
##         opc = OverlayPlotContainer()
##         container.add(opc)
##         opc.add(self.spec_plot)
        container.add(self.spec_plot)
##         container.add(self.map_plot)
        container.add(subcontainer)
##         container.tools.append(SaveTool(container, filename='pic.pdf'))
##         pl1 = Plot(pd,padding=40)
##         line = pl1.plot(("freqs","spectrum"), name="spectrum", index_scale="log")[0]
##         print line
##         opc.add(pl1)
##         
...
##         else:
        ...
        pd.set_data("xpoly",array(sector)[[0,2,2,0,0]])
        pd.set_data("ypoly",array(sector)[[1,1,3,3,1]])
