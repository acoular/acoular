...
def parse_sphinx_searchindex(searchindex):
    ...
    def _parse_dict_recursive(dict_str):
        ...
        while pos >= 0:
            ...
            if dict_str[pos + 1] == '[':
                ...
                msg = 'error when parsing dict: unknown elem'
    ...
    if pos < 0:
        msg = '"objects:" not found in search index'
        raise ValueError(msg)
    ...
    pos = searchindex.find(query)
    if pos < 0:
        msg = '"filenames:" not found in search index'
        raise ValueError(msg)
...
class SphinxDocLinkResolver:
    ...
    def __init__(self, doc_url, searchindex='searchindex.js', extra_modules_test=None, relative=False):
        ...
        if os.name.lower() == 'nt' and not doc_url.startswith('http://'):
            if not relative:
                msg = 'You have to use relative=True for the local package on a Windows system.'
                raise ValueError(msg)
        ...
        self._searchindex = {'filenames': filenames, 'objects': objects}
...
carousel_thumbs = {
    'plot_classifier_comparison_001.png': (1, 600),
...
def generate_example_rst(app):
    ...
    examples.
    """
    root_dir = os.path.join('.', 'auto_examples')
    example_dir = os.path.abspath(os.path.join('.', '..', '..', 'examples'))
    generated_dir = os.path.abspath(os.path.join('.', 'modules', 'generated'))
...
def line_count_sort(file_list, target_dir):
    ...
    index = np.lexsort((unsorted[:, 0].astype(str), unsorted[:, 1].astype(float)))
    if not len(unsorted):
        return []
...
def _thumbnail_div(subdir, full_dir, fname, snippet):
    ...
    ref_name = ref_name.removeprefix('._')
    out = []
    out.append(
        f"""
...
.. raw:: html
    ...
    <div class="thumbnailContainer" tooltip="{snippet}">

"""
    )

    out.append(f'.. figure:: {thumb}\n')
    link_name = link_name.removeprefix('._')
    if full_dir != '.':
        out.append(f'   :target: ./{full_dir}/{fname[:-3]}.html\n\n')
    else:
        out.append(f'   :target: ./{link_name[:-3]}.html\n\n')
    out.append(
        f"""   :ref:`example_{ref_name}`
...
"""
            ...
            for backref in backrefs:
                include_path = os.path.join(root_dir, f'../modules/generated/{backref}.examples')
...
def generate_file_rst(fname, target_dir, src_dir, root_dir, plot_gallery):
    ...
    image_fname = f'{base_image_name}_%03d.png'
    ...
    if plot_gallery and fname.startswith('plot'):
        ...
        if not os.path.exists(first_image_file) or os.stat(first_image_file).st_mtime <= os.stat(src_file).st_mtime:
            ...
            print(f' - time elapsed : {time_elapsed:.2g} sec')
        else:
        ...
        if os.path.exists(first_image_file):
            ...
            if first_img in carousel_thumbs:
                make_thumbnail(
                    (image_path % carousel_thumbs[first_img][0]), carousel_tfile, carousel_thumbs[first_img][1], 190
                )
    ...
    for this_module, url in resolver_urls.items():
        try:
            ...
            print(
                ...
                f'Unable to continue embedding `{this_module}` links due to a URL '
                'Error:\n'
            )
            print(e.args)
